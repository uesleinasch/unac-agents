...
if (isToolEnabled('request_user_input')) {
    ...
    async (args) => {
        ...
        if (answer === '__TIMEOUT__') {
            return {
                content: [
                    { type: 'text', text: 'User did not reply: Check the situation and choose the best option.' },
                ],
            };
        }
...
if (isToolEnabled('message_complete_notification')) {
    ...
    async (args) => {
        ...
        const answer = await getCmdWindowInput(projectName, message, globalTimeoutSeconds, false);
        if (answer === '__TIMEOUT__') {
            return {
                content: [{ type: 'text', text: 'User did not reply: Check the situation and choose the best option.' }],
            };
        }
        if (answer === '') {
            return {
                content: [{ type: 'text', text: 'User acknowledged without a reply.' }],
            };
        }
        return {
