...
const App = ({ sessionId, title, outputDir, timeoutSeconds }) => {
                    ...
                    if (inputExists) {
                        ...
                        try {
                            ...
                            if (typeof inputData === 'object' &&
                                ...
                                logger.error(`Invalid format in ${sessionId}.json. Expected JSON with id (string), text (string), and optional options (array).`);
                            }
                        }
                        catch (parseError) {
    ...
    return (React.createElement(Box, { flexDirection: "column", padding: 1, borderStyle: "round", borderColor: "blue" },
        React.createElement(Box, { marginBottom: 1, flexDirection: "column", width: "100%" },
            React.createElement(Text, { bold: true, color: "magentaBright", wrap: "wrap" }, title),
        ...
        currentQuestionId && (React.createElement(Box, { flexDirection: "column", marginTop: 1, padding: 1, borderStyle: "single", borderColor: timeLeft !== null && timeLeft <= 10 ? 'red' : 'yellow' },
            React.createElement(InteractiveInput, { key: currentQuestionId, 
                ...
                question: chatHistory
                    .slice()
                    .reverse()
                    .find((m) => m.isQuestion && !m.answer)?.text || '', questionId: currentQuestionId, predefinedOptions: currentPredefinedOptions, onSubmit: handleSubmit }),
            timeLeft !== null && (React.createElement(Box, { flexDirection: "column", marginTop: 1 },
                React.createElement(Text, { color: timeLeft <= 10 ? 'red' : 'yellow' },
                    "Time remaining: ",
                    timeLeft,
                    "s"),
