...
sys.exit(5)
`;
function getInputViaGtkTextDialog(projectName, promptMessage, timeoutSeconds) {
    const result = spawnSync('python3', ['-c', GTK_TEXT_DIALOG, projectName, promptMessage, String(timeoutSeconds)], { encoding: 'utf8' });
    if (result.status === 0)
...
function getInputViaGtkRadioDialog(projectName, promptMessage, timeoutSeconds, predefinedOptions) {
...
function getInputViaOsascript(projectName, promptMessage, timeoutSeconds, predefinedOptions) {
    let script;
...
function getInputViaPowerShell(projectName, promptMessage) {
    const script = [
...
// ─── Public API ───────────────────────────────────────────────────────────────
export async function getCmdWindowInput(projectName, promptMessage, timeoutSeconds = USER_INPUT_TIMEOUT_SECONDS, _showCountdown = true, predefinedOptions) {
    const platform = os.platform();
    if (platform === 'linux') {
        if (predefinedOptions && predefinedOptions.length > 0) {
            return getInputViaGtkRadioDialog(projectName, promptMessage, timeoutSeconds, predefinedOptions);
        }
        return getInputViaGtkTextDialog(projectName, promptMessage, timeoutSeconds);
    }
    if (platform === 'darwin') {
        return getInputViaOsascript(projectName, promptMessage, timeoutSeconds, predefinedOptions);
    }
